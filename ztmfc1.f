...
c atmosphere model; steady linear dynamics with nonlinear convergence
c feedback (Ql in A3b ZC1987)
...
701   CONTINUE
      IF(ABS(NINO3) .LE. 0.1) ISTEP=.FALSE. !yanshan yu's thesis Page15
...
310   DO 21 I=6, 25
      ...
      DIVT(I,J)=DD*0.9
      IF(DD .LE. 0.0) GO TO 23
      IF((RDIV(I,J)+DD) .LE. 0.0) GO TO 24
      QM(I,J)=Q0O(I,J)*ALPHA
      GO TO 29
24    QM(I,J)=(-BETA*(RDIV(I,J)+DD)+Q0O(I,J)*ALPHA)
      GO TO 29
23    IF((RDIV(I,J)+DD) .LE. 0.0) GO TO 25
      QM(I,J)=(BETA*DD+Q0O(I,J)*ALPHA)
      GO TO 29
25    QM(I,J)=(-BETA*RDIV(I,J)+Q0O(I,J)*ALPHA)
29    CONTINUE
22    CONTINUE
21    CONTINUE
...
5556  DO 5555 I=6, 25
      ...
      Q0(I,J)=Q0(I,J)*EXP(0.06*(A-29.8)) !Qs in A3a in ZC1987, liang
55    CONTINUE
5555  CONTINUE
...
c taper the northern and southern regions to avoid spurious wave generation
      ...
      DO 32 I=2, 5
      I1=31-I
      HTAU(J,I,K)=.2*FLOAT(I-1)*HTAU(J,6,K)
      HTAU(J,I1,K)=.2*FLOAT(I-1)*HTAU(J,25,K)
32    CONTINUE
31    CONTINUE
30    CONTINUE
